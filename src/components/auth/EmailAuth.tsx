...
const EmailAuth: React.FC<EmailAuthProps> = ({
  ...
  const renderLogin = () => <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 w-full max-w-sm">
        <div>
          ...
          <Input type="email" placeholder="your.email@example.com" value={email} onChange={e => setEmail(e.target.value)} className="py-6" required />
        </div>
        
        <div className="relative">
          <label className="text-sm text-gray-500 dark:text-gray-400 mb-1 block">Password</label>
          <div className="relative">
            <Input type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="pr-10 py-6" required />
            <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400">
              {showPassword ? <EyeOff size={18} /> : <Eye size={18} />}
            </button>
          </div>
        </div>
        
        <Button type="submit" className="w-full py-6 text-lg bg-emerald-500 hover:bg-emerald-600 rounded-xl" disabled={loading}>
          {loading ? "Logging in..." : "Log In"}
        </Button>
        
        <div className="text-center">
          <button type="button" onClick={() => setMode('forgot-password')} className="text-sm text-emerald-500">
        ...
        <div className="flex flex-col space-y-4">
          <div className="flex justify-center space-x-6 mb-2">
            <button type="button" className="p-3 border border-gray-300 dark:border-gray-700 rounded-full text-zinc-300">
          ...
          <Button type="button" variant="outline" onClick={onSwitchToPhone} className="w-full">
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400">
          ...
          <button type="button" onClick={() => setMode('signup')} className="text-emerald-500 ml-1">
            Sign Up
          </button>
        </p>
      </form>
  ...
  const renderSignup = () => <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 w-full max-w-sm">
        <div>
          ...
          <Input type="text" placeholder="John Doe" value={fullName} onChange={e => setFullName(e.target.value)} className="py-6" />
        </div>
        ...
        <div className="relative">
          ...
          <div className="relative">
            <Input type={showConfirmPassword ? "text" : "password"} placeholder="••••••••" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} className="pr-10 py-6" required />
            <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400">
        ...
        <p className="text-center text-sm text-gray-500 dark:text-gray-400">
          ...
          <button type="button" onClick={() => setMode('login')} className="text-emerald-500 ml-1">
            Login
          </button>
        </p>
      </form>
  ...
  const renderForgotPassword = () => <>
      <div className="w-full max-w-sm">
        <button onClick={() => setMode('login')} className="flex items-center mb-4 bg-emerald-500 text-white p-2 rounded-full w-fit">
          <ArrowLeft size={18} />
        </button>
